...
#include <linux/mm.h>
#include <linux/scatterlist.h> // Change!
...
cryptoapi_demo(void)
{
        /* config options */
        char *algo = "aes";
        int mode = CRYPTO_TFM_NEED_KEY;//CRYPTO_TFM_MODE_CBC;
        char key[16], iv[16];

        /* local variables */
        struct crypto_cipher *ciph;//crypto_tfm *tfm;
        ...
        ciph = crypto_alloc_comp(algo, mode);//crypto_alloc_tfm (algo, mode);

        if (tfm == NULL) {
                printk("failed to load transform for %s %s\n", algo, mode == CRYPTO_TFM_NEED_KEY ? "CBC" : "");
                return;
        }

        ret = crypto_cipher_setkey(ciph, key, sizeof(key));//(tfm, key, sizeof(key));
